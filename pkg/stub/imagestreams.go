...
func (h *Handler) processImageStreamWatchEvent(is *imagev1.ImageStream, deleted bool) error {
	...
	if !doUpsert {
		...
		beforeErrorReason := cfg.Condition(v1.ImportImageErrorsExist).Reason
		beforeProgressReason := cfg.Condition(v1.ImageChangesInProgress).Reason
		beforeInReasons := cfg.NameInReason(beforeProgressReason, is.Name) || cfg.NameInReason(beforeErrorReason, is.Name)
		if cache.UpsertsAmount() > 0 {
			...
			if beforeInReasons && (beforeProgressReason != afterProgressReason || beforeErrorReason != afterErrorReason) {
